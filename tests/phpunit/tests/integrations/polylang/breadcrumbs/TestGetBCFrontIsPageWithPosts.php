...
if ( file_exists( ABSPATH . '/wp-content/plugins/polylang/polylang.php' ) && class_exists( 'PLL_UnitTestCase' ) ) {
	 ...
	 */
	class TestGetBCFrontIsPageWithPosts extends PLL_UnitTestCase {
		 ...
		 */
		public function test_get_the_breadcrumbs_with_posts() {
			$this->init_test();
			...
			// ドイツ語の投稿用ページはあえて設定していないため、投稿アーカイブはフロントページに表示されるはず。
			$this->pll_env->curlang = self::$model->get_language( 'de' );
			$this->go_to( home_url( '/de' ) );
