...
if ( file_exists( ABSPATH . '/wp-content/plugins/polylang/polylang.php' ) && class_exists( 'PLL_UnitTestCase' ) ) {
	 ...
	 */
	class TestGetBCFrontIsPostsWithCPTCategoryArchive extends PLL_UnitTestCase {
		 ...
		 * 紐づけたときの振る舞いを理解しておく目的で作った。詳しくは、
		 * {@see TestGetBCCategoryArchive::test_get_the_breadcrumbs_with_category_archive_and_cpt}
		 * のdocコメントを参照のこと。
		 *
		 * @covers ::get_the_breadcrumbs
		 * @preserveGlobalState disabled
		 */
		public function test_get_the_breadcrumbs_with_cpt_category_archive() {
			$this->init_test();

			$post_type_object             = get_post_type_object( 'trcpt' );
			$post_type_object->taxonomies = array( 'category' );
			register_taxonomy_for_object_type( 'category', 'trcpt' );

			$post_type     = 'trcpt';
			$pre_get_posts = function ( $query ) use ( $post_type ) {
				if ( is_category() ) {
					// カテゴリアーカイブの投稿タイプクエリに `trcpt` を追加する。
		 ...
		 * @preserveGlobalState disabled
		 */
		public function test_get_the_breadcrumbs_with_cpt_category_archive_and_has_parent_cat() {
			$this->init_test();
