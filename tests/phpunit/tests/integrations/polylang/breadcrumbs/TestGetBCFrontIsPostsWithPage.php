...
if ( file_exists( ABSPATH . '/wp-content/plugins/polylang/polylang.php' ) && class_exists( 'PLL_UnitTestCase' ) ) {
	 ...
	 */
	class TestGetBCFrontIsPostsWithPage extends PLL_UnitTestCase {
		 ...
		 * @preserveGlobalState disabled
		 */
		public function test_get_the_breadcrumbs_with_page_and_has_parent_page() {
			$this->init_test();
			...
			self::$model->post->set_language( $parent_en, 'en' );

			// フランス語
			$parent_fr = self::factory()->post->create(
				array(
					'post_type'  => 'page',
					'post_title' => 'essai',
				)
			);
			self::$model->post->set_language( $parent_fr, 'fr' );

			// ドイツ語
			$parent_de = self::factory()->post->create(
				array(
					'post_type'  => 'page',
					'post_title' => 'prufen',
				)
			);
			self::$model->post->set_language( $parent_de, 'de' );

			self::$model->post->save_translations( $parent_en, compact( 'parent_en', 'parent_fr', 'parent_de' ) );
			...
			$expected = array(
				...
				array(
					'text'  => 'test',
					'link'  => home_url( user_trailingslashit( '/en/test' ) ),
					'layer' => 'parent_post',
				),
			);
			...
			$expected = array(
				...
				array(
					'text'  => 'essai',
					'link'  => home_url( user_trailingslashit( '/fr/essai' ) ),
					'layer' => 'parent_post',
				),
			);
			...
			$expected = array(
				...
				array(
					'text'  => 'prufen',
					'link'  => home_url( user_trailingslashit( '/de/prufen' ) ),
					'layer' => 'parent_post',
				),
			);
