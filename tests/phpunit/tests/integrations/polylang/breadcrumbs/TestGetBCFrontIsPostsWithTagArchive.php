...
if ( file_exists( ABSPATH . '/wp-content/plugins/polylang/polylang.php' ) && class_exists( 'PLL_UnitTestCase' ) ) {
	 ...
	 */
	class TestGetBCFrontIsPostsWithTagArchive extends PLL_UnitTestCase {
		...
		public function test_get_the_breadcrumbs_with_tag_archive() {
			...
			for ( $i = 0; $i < 9; $i ++ ) {
				...
				wp_set_post_terms( $en, array( $term_en ), 'post_tag' );
			}
			...
			for ( $i = 0; $i < 9; $i ++ ) {
				...
				wp_set_post_terms( $fr, array( $term_fr ), 'post_tag' );
			}
			...
			$this->go_to( get_tag_link( $term_fr ) );
			...
			$this->go_to( get_tag_link( $term_de ) );
