...
if ( file_exists( ABSPATH . '/wp-content/plugins/polylang/polylang.php' ) && class_exists( 'PLL_UnitTestCase' ) ) {
	 ...
	 */
	class TestGetBCFrontIsPostsWithCPTPosts extends PLL_UnitTestCase {
		 ...
		 */
		public function test_get_the_breadcrumbs_with_cpt_posts() {
			$this->init_test();
			...
			$this->go_to( home_url( user_trailingslashit( '/en/trcpt' ) ) );
			...
			$this->go_to( home_url( user_trailingslashit( '/fr/trcpt' ) ) );
			...
			$this->go_to( home_url( user_trailingslashit( '/de/trcpt' ) ) );
